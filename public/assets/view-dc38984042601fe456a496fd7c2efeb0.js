...
$(document).ready(function() {
    ...
    for(var i = 0; i < map_data.landmarks.length; i++){
      ...
      for (var j = 0; j < landmarkprep.length; j++){
        if(landmarkprep[j].landmarkname === map_data.landmarks[i].landmarkname){
          existFlag = true;
        }
      }

      if(existFlag){
        landmarkprep.push({"landmarkname": map_data.landmarks[i].landmarkname + " ("
        + capitalizeFirstLetter(map_data.landmarks[i].img.slice(0,map_data.landmarks[i].img.length-3)) + ") ",
        "id":map_data.landmarks[i].id});
      }
      else
        landmarkprep.push({"landmarkname": map_data.landmarks[i].landmarkname,"id":map_data.landmarks[i].id});
    }

    for(var i = 0; i < landmarkprep.length; i++){
      console.log(landmarkprep[i]);
      to_append = "<div class='checkbox'><label>&nbsp;&nbsp;&nbsp;<input type='checkbox' value='"+landmarkprep[i].id+"'>&nbsp;&nbsp;";
      to_append += landmarkprep[i].landmarkname;
      to_append += "</label></div>";
